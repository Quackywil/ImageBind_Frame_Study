...
% Precompute the aspect ratio of the rectangle
rectAspectRatio = imageWidth / imageHeight;
...
for trialNumber = 1:length(dataMatrix)
    ...
    bestAspectRatio = size(best_image, 2) / size(best_image, 1);
    worstAspectRatio = size(worst_image, 2) / size(worst_image, 1);

    % Adjust the image dimensions to fit the rectangles without distorting the aspect ratio
    if bestAspectRatio > rectAspectRatio
        % Image is wider, adjust the height
        best_image = imresize(best_image, [imageWidth / bestAspectRatio, imageWidth]);
    else
        % Image is taller or has the same aspect ratio, adjust the width
        best_image = imresize(best_image, [imageHeight, imageHeight * bestAspectRatio]);
    end

    if worstAspectRatio > rectAspectRatio
        % Image is wider, adjust the height
        worst_image = imresize(worst_image, [imageWidth / worstAspectRatio, imageWidth]);
    else
        % Image is taller or has the same aspect ratio, adjust the width
        worst_image = imresize(worst_image, [imageHeight, imageHeight * worstAspectRatio]);
    end
    ...
    image1Rect = [screenWidth / 2 - spaceBetweenImages - size(best_image, 2), ...
        (screenHeight - size(best_image, 1)) / 2, ...
        screenWidth / 2 - spaceBetweenImages, ...
        (screenHeight + size(best_image, 1)) / 2];

    image2Rect = [screenWidth / 2 + spaceBetweenImages, ...
        (screenHeight - size(worst_image, 1)) / 2, ...
        screenWidth / 2 + spaceBetweenImages + size(worst_image, 2), ...
        (screenHeight + size(worst_image, 1)) / 2];
