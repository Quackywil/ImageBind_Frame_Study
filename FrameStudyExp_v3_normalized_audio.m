...
dataMatrix = create_dataMatrix(AudioFilesSet, best_image, worst_image, img_key, binding_diff, binding_max, binding_min);
...
%     end
    ...
    dataMatrix{trialNumber, 8} = response; % Participant's responses
    dataMatrix{trialNumber, 9} = bestTexturePosition; % Best Image Key

    % Check if the Response matches the BestImage_Key and update the 'CorrectAnswers' field
    if strcmp(dataMatrix{trialNumber, 8}, dataMatrix{trialNumber, 9}) % 1 = Correct; 0 = Incorrect
        dataMatrix{trialNumber, 10} = 1; % 1 for correct
    else
        dataMatrix{trialNumber, 10} = 0; % 0 for incorrect
    end
    dataMatrix{trialNumber, 11} = responseTime; % Reaction time
end

dataMatrix(:, 12) = ID_cell;
