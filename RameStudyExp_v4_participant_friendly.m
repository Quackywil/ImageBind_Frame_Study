...
ID_cell = cell(100, 1); % Column Vector
...
% IQA_scores = NR_IQA_scores(AudioFilesSet, best_image, worst_image);

dataMatrix = create_dataMatrix(AudioFilesSet, best_image, worst_image, img_key, binding_diff, binding_max, binding_min, IQA_scores);
...
%     end
    ...
    dataMatrix{trialNumber, 14} = response; % Participant's responses
    dataMatrix{trialNumber, 15} = bestTexturePosition; % Best Image Key

    % Check if the Response matches the BestImage_Key and update the 'CorrectAnswers' field
    if strcmp(dataMatrix{trialNumber, 14}, dataMatrix{trialNumber, 15}) % 1 = Correct; 0 = Incorrect
        dataMatrix{trialNumber, 16} = 1; % 1 for correct
    else
        dataMatrix{trialNumber, 16} = 0; % 0 for incorrect
    end
    dataMatrix{trialNumber, 16} = responseTime; % Response time
end

dataMatrix(:, 17) = ID_cell;
